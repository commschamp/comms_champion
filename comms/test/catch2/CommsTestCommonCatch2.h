...
{
    REQUIRE(expLength == frame.length(msg));
}
    ...
    comms::ErrorStatus expectedEs = comms::ErrorStatus::Success)
{
    ...
    REQUIRE(std::equal(buf, buf + actualBufSize, static_cast<const char*>(&outCheckBuf[0])));
    return msg;
}
    ...
    comms::ErrorStatus expectedEs = comms::ErrorStatus::Success)
{
    ...
    auto actualBufSize = static_cast<std::size_t>(std::distance(buf, readIter));
    REQUIRE(actualBufSize == stack.length(*msg));
    std::unique_ptr<char []> outCheckBuf(new char[actualBufSize]);
    auto writeIter = comms::writeIteratorFor(msg, &outCheckBuf[0]);
    ...
    comms::ErrorStatus expectedEs = comms::ErrorStatus::Success)
{
    ...
    REQUIRE(es == comms::ErrorStatus::Success);
    REQUIRE(outCheckBuf.size() == actualBufSize);
    REQUIRE(outCheckBuf.size() == stack.length(*msg));
    ...
    comms::ErrorStatus expectedEs = comms::ErrorStatus::Success)
{
    ...
    REQUIRE(std::equal(constBuf, constBuf + bufSize, &expectedBuf[0]));
